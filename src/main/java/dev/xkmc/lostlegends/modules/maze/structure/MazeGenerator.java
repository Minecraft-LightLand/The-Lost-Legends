...
import dev.xkmc.lostlegends.modules.maze.logic.generator.IRandom;
import dev.xkmc.lostlegends.modules.maze.logic.generator.MazeConfig;
import dev.xkmc.lostlegends.modules.maze.logic.generator.MazeGen;
import dev.xkmc.lostlegends.modules.maze.logic.objective.LeafMarker;
import dev.xkmc.lostlegends.modules.maze.logic.objective.MazeIterator;
import dev.xkmc.lostlegends.modules.maze.logic.objective.MazeRegistry;
...
public class MazeGenerator {
	...
	public enum RoomCellType {
		...
		public String getID(String type) {
			return type + "_" + name().toLowerCase(Locale.ROOT);
		}
	}
	...
	private final MazeStructure parent;
	public final int cellWidth, cellHeight, mazeWidth, mazeHeight;

	public MazeGenerator(MazeStructure parent) {
		this.parent = parent;
		this.rooms = parent.rooms;
		this.cellWidth = parent.cellWidth;
		this.cellHeight = parent.cellHeight;
		this.mazeWidth = parent.mazeWidth;
		this.mazeHeight = parent.mazeHeight;
	}
	...
	public CellInstance parseCell(MazeGen maze, int[] sets, LeafMarker[][] values, int x, int z) {
		...
		String room = rooms[values[x][z].level == 0 ? sets[values[x][z].getColor()] : 0];
	...
	public void addPieces(StructureTemplateManager manager, BlockPos pos, List<StructurePiece> children, WorldgenRandom r, MazeConfig conf) {
		MazeGen[] mazes = new MazeGen[mazeHeight];

		// maze body
		for (int i = 0; i < mazeHeight; i++) {
			mazes[i] = new MazeGen(mazeWidth, new IRandom.CRandom(r::nextInt, r::nextDouble), conf, new MazeGen.Debugger());
			mazes[i].gen();
		...
		// roof edge
		int start_pos = -mazes[2].r * cellWidth - 1;
		int end_pos = (mazes[2].w - mazes[2].r) * cellWidth;
		for (int i = 0; i < mazes[2].w; i++) {
			int ix = (i - mazes[2].r) * cellWidth;
			addEdges(manager, children, pos, end_pos, ix, Rotation.NONE, i == mazes[1].r);
			addEdges(manager, children, pos, start_pos, ix + cellWidth - 1, Rotation.CLOCKWISE_180, i == mazes[1].r);
			addEdges(manager, children, pos, ix + cellWidth - 1, end_pos, Rotation.CLOCKWISE_90, i == mazes[1].r);
	...
	private void addEdges(StructureTemplateManager manager, List<StructurePiece> children, BlockPos pos, int x, int z, Rotation rot, boolean door) {
		...
		children.add(new MazePiece(parent, manager, new CellInstance(door ? EDGE_DOOR : EDGE_WALL, rot, Mirror.NONE),
	...
	private void addCorners(StructureTemplateManager manager, List<StructurePiece> children, BlockPos pos, int x, int z, Rotation rot) {
		...
		children.add(new MazePiece(parent, manager, new CellInstance(EDGE_CORNER, rot, Mirror.NONE),
