...
public abstract class AnimalSlime extends BaseSlime {
	...
	public AnimalSlime(EntityType<? extends BaseSlime> type, Level level) {
		super(type, level);
	}
	...
	public abstract Ingredient getFoodIngredient();

	public abstract ItemStack getParticleStack();
	...
	public void onDeathSplit(List<Mob> children) {
		...
		if (count <= 0) return;
		var lastSource = getLastDamageSource();
		if (level() instanceof ServerLevel sl && lastSource != null) {
			for (int i = 0; i < count; i++) {
				dropAllDeathLoot(sl, lastSource);
			}
		}
	}
